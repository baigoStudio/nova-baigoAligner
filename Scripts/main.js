...
var baigoAligner = {
  align: function(selectedText, symbol, remove){
    ...
    return this.getText(arr_selectedText, symbol, remove);
  },
  ...
  getText: function(selectedText, symbol, remove) {
    var str_newText  = '';
    var num_longest  = this.getLongest(selectedText, symbol);
    ...
    for (var str_selectedLine of selectedText) {
      ...
      if (str_selectedLine.indexOf(symbol) > 0) {
        str_newLine += this.getLine(str_selectedLine, symbol, num_longest, remove);
      } else {
  ...
  getLine: function(selectedLine, symbol, longest, remove) {
    ...
    arr_textItem[0] = this.getTrim(arr_textItem[0], 'right');
